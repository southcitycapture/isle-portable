...
MxResult LegoAnimationManager::LoadWorldInfo(LegoOmni::World p_worldId)
{
	...
	if (m_worldId != p_worldId) {
		...
		MxU32 version;
		if (storage.ReadLE(version) == FAILURE) {
			goto done;
		}
		...
		if (storage.ReadLE(m_animCount) == FAILURE) {
			goto done;
		}
...
MxResult LegoAnimationManager::ReadAnimInfo(LegoStorage* p_storage, AnimInfo* p_info)
{
	...
	if (p_storage->ReadLE(length) == FAILURE) {
		goto done;
	}
	...
	if (p_storage->ReadLE(p_info->m_objectId) == FAILURE) {
		goto done;
	}

	if (p_storage->ReadLE(p_info->m_location) == FAILURE) {
		goto done;
	}
	if (p_storage->ReadLE(p_info->m_unk0x0a) == FAILURE) {
		goto done;
	}
	if (p_storage->ReadLE(p_info->m_unk0x0b) == FAILURE) {
		goto done;
	}
	if (p_storage->ReadLE(p_info->m_unk0x0c) == FAILURE) {
		goto done;
	}
	if (p_storage->ReadLE(p_info->m_unk0x0d) == FAILURE) {
		goto done;
	}

	for (i = 0; i < (MxS32) sizeOfArray(p_info->m_unk0x10); i++) {
		if (p_storage->ReadLE(p_info->m_unk0x10[i]) != SUCCESS) {
			goto done;
		}
	}

	if (p_storage->ReadLE(p_info->m_modelCount) == FAILURE) {
		goto done;
	}
...
MxResult LegoAnimationManager::ReadModelInfo(LegoStorage* p_storage, ModelInfo* p_info)
{
	...
	p_info->m_name[length] = 0;
	if (p_storage->ReadLE(p_info->m_unk0x04) == FAILURE) {
		goto done;
	}
	...
	if (p_storage->ReadLE(p_info->m_unk0x2c) == FAILURE) {
		goto done;
	}
