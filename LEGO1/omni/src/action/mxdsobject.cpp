...
MxDSObject* MxDSObjectList::FindInternal(MxDSObject* p_action, MxBool p_delete)
{
	...
	if (p_delete && found != NULL) {
		// Erase the node that was found: after an unknown24 == -3 scan the
		// loop iterator sits at end(), and erasing end() corrupts the list.
		erase(foundIt);
	}
...
MxDSObject* CreateStreamObject(MxDSFile* p_file, MxS16 p_ofs)
{
	...
	if (p_file->Read((MxU8*) &tmpChunk.ckid, 8) == 0 && MxSwapLE(tmpChunk.ckid) == FOURCC('M', 'x', 'S', 't')) {
		if (p_file->Read((MxU8*) &tmpChunk.ckid, 8) == 0 && MxSwapLE(tmpChunk.ckid) == FOURCC('M', 'x', 'O', 'b')) {
			tmpChunk.cksize = MxSwapLE(tmpChunk.cksize);
